...
Membuat data panel IBS dari tahun 2012 ke 2014
...
def analisis_dasar_csv(tahun):
    ...
    # Import Data
    ibs_csv = pd.read_csv(f'Data/ibs_{tahun}.csv',
                          low_memory=False)
    ...
    klasifikasi = ibs_copy[f'disic5{dua_digit_tahun}']
    renum = ibs_copy['psid']
...
def klasifikasi_kolum(df_tahun):
    ...
    kolom_terpilih = ['Tahun', 'Kode Klasifikasi', 'PSID', 'Total Pekerja',
    ...
    df_suffixed.rename(columns={f'PSID_{tahun}': 'PSID'}, inplace=True)
    
    return df_suffixed

# 2014
df_2014 = analisis_dasar_csv('2014')
industri_2014 = analisis_industri(df_2014)
df_2014_selected = klasifikasi_kolum(df_2014)

# 2013
df_2013 = analisis_dasar_csv('2013')
industri_2013 = analisis_industri(df_2013)
df_2013_selected = klasifikasi_kolum(df_2013)

# 2012
df_2012 = analisis_dasar_csv('2012')
industri_2012 = analisis_industri(df_2012)
df_2012_selected = klasifikasi_kolum(df_2012)

# Creating long format panel from 2012-2014
psid_2012 = df_2012_selected['PSID']
len(psid_2012) == len(df_2012_selected['PSID'].unique()) # Kalau hasilnya True, artinya renum bersifat unik

# Merge dataframes from 2015-2019
merged_df = pd.merge(pd.merge(df_2012_selected, df_2013_selected, on='PSID'), df_2014_selected, on='PSID')
nk_merged = merged_df.columns

list_tahun = ['2012', '2013', '2014']

for t in list_tahun:
    df_x = merged_df.filter(regex=f'_{t}')
    df_x['PSID'] = merged_df['PSID']
